...
{
  table->AddDriver("vfhspark",  VFH_Init);
}
...
int VFH_Class::SetupOdom()
{
  ...
  SetRobotRadius( vfh_Algorithm, robot_radius * 1000 );
...
//printf("Command: speed: %d turnrate: %d\n", cmd_speed, cmd_turnrate);
  ...
  else
  {
    if(std::abs(this->con_vel[2]) >
       (double)GetMaxTurnrate(vfh_Algorithm, (int)this->con_vel[0]))
    {
...
void VFH_Class::DoOneUpdate()
{
  ...
  else if (dist > (this->dist_eps * 1e3))
  {
    ...
    if((dist < (this->dist_eps * 1e3 * 2.0)) &&
       (this->speed > (GetCurrentMaxSpeed(vfh_Algorithm) / 2.0)))
    {
//        int foo = this->speed;
      this->speed =
  ...
  else
  {
    ...
    turnrate = (int)rint(std::abs(angdiff/180.0) *
                         GetMaxTurnrate(vfh_Algorithm,speed));
    ...
    if(turnrate < 0)
      turnrate = MIN(turnrate,- GetMinTurnrate(vfh_Algorithm));
    else
      turnrate = MAX(turnrate,+ GetMinTurnrate(vfh_Algorithm));
  ...
  : ThreadedDriver(cf, section, true, PLAYER_MSGQUEUE_DEFAULT_MAXLEN)
{
  ...
  vfhinit();
  this->vfh_Algorithm = create_vfh_ptr   (cell_size,
                                          window_diameter,
                                          sector_angle,
                                          safety_dist_0ms,
                                          safety_dist_1ms,
