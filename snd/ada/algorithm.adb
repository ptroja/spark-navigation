...
package body Algorithm is
   ...
   begin
      ...
      loop
         ...
         begin
            -- deltaAngle in (-Pi;+Pi>
            if abs(deltaAngle) > M_PI/2.0
            then
               -- Semi-circle behind sensor
               if bDoRearCheck and then scans(i).first < width/2.0
               then
                  if gDebug > 7 then
      ...
      begin
         ...
         loop
            ...
            begin
               ...
               if deltaAngle > 0.0 and then deltaAngle < M_PI/2.0
               then
                  if This.laserScan(i).first < This.robot.Max_Range - 0.01
                  then
                     if dist(posGapCorner,Create(This.laserScan(i).first, This.laserScan(i).second)) < This.robot.min_Gap_Width
                     then
                        if gDebug > 4 then
                           Put("    Gap at ");
   ...
   begin
      ...
      loop
         ...
         begin
            ...
            if (dist >= minGapWidth) or else (rayL.first = fMaxRange and then rayR.first < fMaxRange)
            then
               Append(gapVec, Create(rayR.second, rayR.first, -1));
            elsif (dist <= -minGapWidth) or else (rayR.first = fMaxRange and then rayL.first < fMaxRange)
            then
   ...
   procedure removeDuplicateGaps (This : in out Controller) is
      ...
      begin
         ...
         if iterR /= No_Element and then Length(This.gapVec) > 1
         then
            iterL := First(This.gapVec);
      ...
      begin
         ...
         if riterL /= First(This.gapVec) and then Length(This.gapVec) > 1
         then
            riterR := Last(This.gapVec);
   ...
   begin
      ...
      loop
         ...
         begin
            ...
            if Element(This.gapVec,iR).iDir = -1
            then
               if isRisingGapSafe(This,Element(This.gapVec,iR))
               then
            ...
            if Element(This.gapVec,iL).iDir = 1
            then
               if isRisingGapSafe(This,Element(This.gapVec,iL))
               then
                  if Candidate_Valley_IDS.Opt = O_SOME
                  then
...
--                    pragma Assert(iBestValleyRising <= Integer(Length(This.gapVec)));
   ...
   begin
      ...
      loop
         ...
         if This.laserScan(i).first <= safetyDist + This.robot.robot_Radius
         then
      ...
      if deltaAreaSum > 0.0
      then
         obstacleAvoidDelta := obstacleAvoidDelta / deltaAreaSum;
      else
         obstacleAvoidDelta := 0.0;
      end if;
   ...
   begin
      ...
      if iNumLPs = 0 or else iNumLPs > 100000
      then
      ...
      if norm(pos(relGoal)) <= This.robot.goal_Position_Tol
      then
         if almostEqual(ori(robotPose), ori(goal), This.robot.goal_Angle_Tol)
         then
            if gDebug > 0 then
            ...
            if ccwDiff(ori(robotPose), ori(goal)) < M_PI
            then
      ...
      loop
         ...
         if This.laserScan(i).first <= This.robot.robot_Radius
         then
            if gDebug > 0 then
         ...
         if This.laserScan(i).first <= This.robot.obstacle_Avoid_Dist + This.robot.robot_Radius
         then
...
--              --}
            ...
            if pBestValley.value.risingGap.iDir > 0
            then
      ...
      if dCastPi(This.driveAngle) > M_PI/2.0
      then
         This.driveAngle := This.driveAngle + Create(This.obsAvoidDelta);
         if dCastPi(This.driveAngle) < 0.0
         then
            This.driveAngle := Create(M_PI/2.0);
         end if;
      elsif dCastPi(This.driveAngle) < -M_PI/2.0
      then
         This.driveAngle := This.driveAngle + Create(This.obsAvoidDelta);
         if dCast(This.driveAngle) > 0.0
         then
            This.driveAngle := Create(-M_PI/2.0);
         end if;
      else
